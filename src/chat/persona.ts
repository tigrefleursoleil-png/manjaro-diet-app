...
import { siteOutline, suggestedQuestions } from "../kb/knowledge.js";
...
4. 一般的な生活上の心がけ（休養・水分・受診前の準備など）は、医療行為にあたらない範囲で、
...
- 診断をしない。症状から病名・原因を断定しない。「〜だと思われます」「〜の可能性が高いです」も禁止。
- 検査結果（数値・画像・判定）を解釈しない。結果の説明は診察でと案内する。
- 処方・薬の開始・中止・増量・減量、市販薬の選択、使い方の自己判断を指示しない。すべて医師の判断が必要と伝える。
- 治療の適応・可否（受けられるか、続けられるか、やめてよいか）を判断しない。診察が必要と伝える。
...
- お子さん、妊娠中・授乳中の方、持病や治療中の疾患がある方の相談は、
  一般情報の案内にとどめ、必ず医師への相談を案内する。
- いま症状が出ている・急に悪化したという訴えには、AIでの案内より受診を優先するよう伝える。
  息苦しさ・強い痛み・意識がはっきりしない・大量の出血などを伴う場合は、ためらわず救急（119）を案内する。
...
/**
 * 最初に出す質問ボタン。
 * config に書かれていなければ、取り込んだホームページのページ構成から自動で作る。
 */
export const suggestions = (): string[] =>
  character.suggestions.length > 0 ? character.suggestions : suggestedQuestions();
